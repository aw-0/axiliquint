<template>
    <div id="hi there inspect-elementer">
        <div class="px-8">
            <div class="w-full mx-auto text-left mt-10">
                <h1 class="text-4xl text-white font-extrabold">☎️ Want to contact me?</h1>
                <h2 class="text-3xl text-white font-semibold mt-4">Sounds good, let's <span class="font-extrabold magical-gradient">start chatting</span>.</h2>
            </div>
            <MagicalBar class="mb-4" />
            <div class="w-full mx-auto">
                <h1 class="text-2xl text-center text-white font-semibold mb-4">Pricing</h1>
                <p class="text-center text-white mb-2">Pricing is highly dependent on the time commitment I will have to put foward on your project.</p>
                <p class="text-center text-white mb-2">I can either be paid by asset, or hourly. I accept Robux and USD.</p>
                <p class="text-center text-white">My time is valuable, and I'm sure yours is too. Don't bother requesting me for non-serious projects.</p>
            </div>
            <MagicalBar class="mb-4" />
            <div class="mx-auto">
                <h1 class="text-2xl text-center text-white font-semibold mb-4">Ways to Contact</h1>
                <div class="sm:flex items-center justify-center mt-5">
                    <a href="https://twitter.com/axiliquint" target="_blank" class="px-5 py-3 border border-transparent text-base rounded-md sm:mx-2 text-white font-medium" style="background: #1c95de">
                        <i class="fab fa-twitter"></i> DM me on Twitter
                    </a>
                    <div class="my-8 sm:my-0"></div>
                    <button @click="changeDiscBtnText" v-html="discButtonText" class="px-5 py-3 border border-transparent rounded-md sm:mx-2 text-white font-medium" style="background: #7289DA">
                    </button>
                    <div class="my-8 sm:my-0"></div>
                    <a href="mailto:andrew@koal.us" class="px-5 py-3 border border-transparent rounded-md sm:mx-2 text-white font-medium" style="background: #7F7F7F">
                        <i class="fas fa-envelope"></i> Email me
                    </a>
                    <div class="mb-8 sm:my-0"></div>
                </div>
            </div>
        </div>
        <FooterWave style="margin-top: 0.1px" />
    </div>
</template>

<script>
export default {
    data(){
        return {
            discButtonText: '<i class="fab fa-discord"></i> Talk with me on Discord'
        }
    },
    methods: {
        changeDiscBtnText(){
            this.discButtonText = '<i class="fab fa-discord"></i> Add me: <p class="font-extrabold font-blue-500">and#1000</p>'
        },
        sleep(ms) {
            return new Promise(resolve => setTimeout(resolve, ms));
        }
    }

}
</script>

<style>

</style>